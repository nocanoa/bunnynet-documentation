...
The credentials you need to connect via S3 are located in the **FTP & API Access** tab:

- **Access Key ID**: Your unique identifier (Storage Zone Read Only Password)
- **Secret Access Key**: Your authentication credential (Storage Zone Password)
...
<Note>
  ...
  `https://[region]-s3.storage.bunnycdn.com/bucket-name/key`). Virtual
  hosted-style URLs (e.g., `bucket-name.s3.region.amazonaws.com`) are not
  supported.
</Note>
...
| ✅ [`AbortMultipartUpload`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_AbortMultipartUpload.html)       | ✅ Cancels multipart upload<br />✅ Deletes all uploaded parts<br />                                                                                                                                                                                                                                                                                                                                                                       |
...
| API Name                                                                                     | Feature                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ [`HeadObject`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_HeadObject.html)       | ✅ Basic metadata retrieval<br />✅ Content-Type header<br />✅ Content-Length header<br />✅ Last-Modified header<br />❌ ETag<br />❌ Conditional Operations (If-Match, If-Modified-Since, etc.)<br />❌ Range queries<br />❌ SSE-C encryption headers                                                                                                                                                                                                                                                                                   |
| ✅ [`GetObject`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_GetObject.html)         | ✅ Full object download<br />✅ Content-Type header<br />✅ Content-Length header<br />✅ Range requests (byte ranges)<br />✅ Multipart object download<br />❌ Conditional Operations (If-Match, If-Modified-Since, etc.)<br />❌ SSE-C encryption<br />❌ Versioning support                                                                                                                                                                                                                                                             |
| ✅ [`PutObject`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutObject.html)         | ✅ Object upload<br />✅ System Metadata:<br />✅ Content-Type (with automatic detection)<br />✅ Content-Length<br />✅ Checksum validation:<br />✅ x-amz-checksum-sha256 (Base64 encoded)<br />✅ Automatic replication to configured zones<br />❌ Storage Class selection<br />❌ Cache-Control, Content-Disposition, Content-Encoding<br />❌ Expires, Content-Language<br />❌ SSE encryption<br />❌ SSE-C encryption<br />❌ Object tagging (x-amz-tagging)<br />❌ Object locking<br />❌ ACL headers (x-amz-acl, x-amz-grant-\*) |
...
| ✅ [`CopyObject`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CopyObject.html)       | ✅ Object copy within same storage zone<br />✅ LastModified in response<br />✅ ChecksumSHA256 in response (Base64 encoded)<br />✅ Content-Type preservation<br />✅ Source object checksum validation<br />❌ ETag in response<br />❌ Cross-zone copying<br />❌ Metadata directive (x-amz-metadata-directive)<br />❌ Conditional copy operations<br />❌ Storage class selection<br />❌ SSE/SSE-C encryption<br />❌ Tagging                                                                                                         |
| ✅ [`ListObjects`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_ListObjects.html)     | ✅ Query Parameters:<br />✅ prefix<br />✅ delimiter<br />✅ max-keys (default 1000)<br />✅ marker (pagination)                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
<Note>S3 compatibility cannot be changed after storage zone creation.</Note>

### Known limitations

The following S3 features are not currently supported:

- **Pre-signed URLs**: Temporary access links for time-limited object access
...
<Note>
  Some error codes may differ from standard AWS S3 behavior. See the notes below
  for Bunny-specific details.
</Note>
...
1. **Use ListObjectsV2** instead of ListObjects for better performance and features
2. **Implement retry logic** for transient failures
3. **Use multipart uploads** for objects larger than 100MB
4. **Include Content-Type headers** when uploading, or rely on automatic detection
5. **Handle pagination properly** when listing objects or parts (max 1000 per request)
6. **Monitor multipart sessions** and abort abandoned uploads to free storage
