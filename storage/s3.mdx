...
<Info>
  S3 compatibility is currently in beta. S3 compatibility must be enabled during storage zone creation and cannot be changed afterward.
</Info>
...
<Note>
  You must use `--endpoint-url https://[region]-s3.storage.bunnycdn.com` with each command or set up an [AWS CLI
  profile](https://docs.aws.amazon.com/cli/v1/userguide/cli-configure-files.html#cli-configure-files-methods) with the endpoint configured.
</Note>
